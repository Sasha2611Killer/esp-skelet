...
local function update3DBox(esp, character)
    ...
    -- Базовые размеры
    local width = 2
    local height = humanoid.HipHeight * 1.5
    local depth = 1.5

    -- Корректировка для разных типов персонажей
            ...
            if rootPart and humanoid and head then
                ...
                if headOnScreen then
                    ...
                    if ESP_SETTINGS.ShowBox then
                        if not update3DBox(esp, character) then
                            for i = 1, 12 do
                    ...
                    if ESP_SETTINGS.ShowHealth then
                        ...
                        local healthBarY = rootPos.Y + 40
                        
                        esp.health.From = Vector2.new(rootPos.X - 30, healthBarY)
                        esp.health.To = Vector2.new(rootPos.X - 30, healthBarY - 80 * healthPercentage)
